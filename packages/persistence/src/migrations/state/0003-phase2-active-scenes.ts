 ...
 *   生命周期 Record；payload 可信（v1 + 字段形态）才采用其 to/cycleId，
 *   否则 sceneId 回退 aggregate_id 前缀、state 保守落 unknown（未证
 *   终态）；可证终态不产生行。
 * - SQL 细节：SQLite 不保证 AND 短路求值，json_* 对非法 JSON 会抛错
 *   ——全部 JSON 读取都包在 CASE WHEN json_valid(...) 惰性分支内；
 *   (occurred_at_ms, record_id) 决胜取最新行；静态 SQL，无拼接通道。
 */
...
FROM (
  SELECT session_id, scene_id, cycle_id, state, occurred_at_ms,
    ROW_NUMBER() OVER (
      ...
      ORDER BY occurred_at_ms DESC, record_id DESC
    ) AS rn
  FROM (
    SELECT session_id, record_id, occurred_at_ms,
      CASE
    ...
    FROM (
      SELECT session_id, record_id, occurred_at_ms, aggregate_id,
        CASE
          WHEN payload_version = 1
            AND typeof(payload_scene_id) = 'text'
            AND typeof(payload_cycle_id) = 'text'
            AND typeof(payload_from) = 'text'
            AND typeof(payload_to) = 'text'
            AND length(payload_scene_id) = 36
            AND length(payload_cycle_id) = 36
