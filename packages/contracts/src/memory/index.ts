...
export const ContextContributionSchema = extensibleJsonObject({
  ...
  priorityDerivationVersion: z.number().int().positive(),
  blocks: z.array(ContextBlockSchema),
  returnedBlockIds: z.array(z.string().min(1).max(256)),
  sourceWatermark: DecimalStringSchema,
  completedRoutes: z.array(z.string().min(1).max(128)),
  degradedRoutes: z.array(MemoryDegradationSchema),
  partial: z.boolean(),
  cacheUntil: z.number().int().nonnegative().nullable(),
  nextWakeAt: z.number().int().nonnegative().nullable(),
  personaRevision: DecimalStringSchema,
  personaContentHash: z.string().max(256),
...
export const MemoryProviderCapabilitiesSchema = extensibleJsonObject({
  ...
  persona: z.boolean(),
  activeSurfaceMode: z.enum(["off", "advisory", "required"]),
