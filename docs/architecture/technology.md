...
各自应用携带通过 CI 的语言运行时；Bellis 继续分发官方 Node 二进制，不使用 Node SEA。更细的交付矩阵见 [路线图](../plans/README.md)。
...
当前实施拆分、协议 Gate 与验收标准见 [Phase 2 开发指南](../archive/phase-2/development-guide.md)。

- 用 Fake Signal 和 Fake Model 产生 ActionFrame。
- 打通 Action Compiler、Scene Director 和 Cue Timeline。
- 实现 Stage、AudioWorklet、字幕和 Recording/DOM Avatar 基础；真实 Cubism 由 L5 交付。
- 验证语音、字幕和 Live2D 起始偏差。

### 阶段三：Decision Loop

当前实施拆分、契约 Gate 与验收标准见 [Phase 3 开发指南](../archive/phase-3/development-guide.md)。
...
当前实施拆分、契约 Gate 与验收标准见 [Phase 4 构建指南](../plans/phase-4/README.md)。
...
- Phase 4B 多 Provider/MCP、Avatar Mixer/Presence、资源仲裁及恢复待办尚未完成；其技术要求保留，执行由后续 L5–L7 承接。

### 阶段五至八：直播与游戏平台双线交付

以下均为待实施排期。总入口为 [双线路线图](../plans/README.md)，当前开工入口为 [Phase 5](../plans/phase-5.md)。

| 阶段 | 直播线 | 游戏线 | 共同验收 |
| --- | --- | --- | --- |
| Phase 5 | 真实 LLM/TTS、Cubism/Mixer/Presence、首个平台、最小 Studio、无游戏 OBS | 公共 Plugin SDK、Activity、独立 Runtime/Adapter SDK/FakeGame/Client/薄插件 | J5 真实表现 + FakeGame，同一语音/记忆通道 |
| Phase 6 | 降级/接管、负载与取消指标、必要恢复、最小运维/Trace | Windows 原神、真实 Broker、战斗与完整任务链 | J6 首款真实游戏直播 |
| Phase 7 | 多 Provider/MCP、完整角色策略、剩余 Phase 4B、长时运行/诊断 | 第二真实游戏、能力与包隔离、切换和配对跨机 | J7 多游戏、断网恢复与长时直播 |
| Phase 8 | 完整 Studio、Launcher、安装/签名更新/回滚 | 独立游戏应用、managed、包/模型组合更新 | J8 干净安装与发布矩阵 |

实现细节分别由 [直播线](../plans/live/README.md) 和 [游戏线](../plans/game/README.md) 维护。Phase 4A 冻结不变；Phase 4B 按 [承接清单](../plans/phase-4/backlog.md) 在直播线补齐。游戏包拆仓按需决定，不强制新增阶段。
