...
P0 的监督组件是 SupervisionRecord、ExecutionGrant、首验准入、StopOperation 生命周期、资源隔离与独立清理记录的唯一语义写入者；宿主写效果登记与带来源投影，假设备只写本实例的实际效果、水位、停止和清理事实。持久化 Worker 是 SQLite 的唯一物理写入者，不替代这些事实权威。安全通道与业务通道都把对端实际身份绑定到连接，不能从消息中的 operator_id、role 或 instance 字符串推导权限。
...
P0 宿主、监督与假设备分别运行在独立 Node 进程；同步 SQLite 位于监督进程的独立 Worker。监督拥有独立租约定时器、直连端点安全通道及终态后的清理责任，不依赖宿主循环转发停止。端点同时执行本地有限租约，监督失联也不能无限继续。新增效果需等待事务回执，并在回执后重新检查仍有效的许可；撤权、停止与 UNKNOWN 先推进本地栅栏，异步记录失败不得将其回滚。
...
P0 只加载第20.2节准入的受信假设备。宿主与监督入口位于 apps/host/，运行逻辑位于 packages/runtime/，假设备在 plugins/ 下独立实现，本地操作员 CLI 使用受限 Unix domain socket 管理通道。监督与端点使用另一条直连安全通道；本机目录和 socket 权限、安装身份和连接凭据共同限制访问。插件只持有自身业务/安全通道身份，不持有操作员管理凭据，不继承真实设备或供应商凭据。没有这些职责的空包不预建。
