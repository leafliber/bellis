...
P0 的 connection.announced 是连接内、无效果的身份/时钟引导公告，不进入可靠业务 Outbox，也不携带许可或执行指令。其封闭载荷和签名绑定服务实例、连接、一次性挑战及目标时钟；接收者先验证受信服务公钥，再采用时钟证据。实际效果与清理事件仍按登记的可靠性处理；存储不可用时先安全收权并显式记录缺口，不能为了落盘阻塞停止。
