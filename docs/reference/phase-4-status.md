...
冻结恢复证据使用 Core 0.13.0 / Schema 20、SDK 0.11.2，记录 178 个 Core 安装文件及 4 个 SDK 文件核验。精确安装物 SHA、命令和退出码见 [冻结摘要](../evidence/phase4a-recovery-scope-freeze.json)。支持范围只以实测安装组合为准；当前运行入口为 `pnpm start:iris`，配置、凭据和 readiness 见 [运行说明](../guides/iris-runtime.md)。
...
真实 Chromium、Runtime/DB Worker、已安装 SDK 与 Core 的 100 Cycle 连续链路已留有证据，见 [连续验收](../validation/iris-continuous.md)。Stage 效果事务另有两个 Runtime 崩溃窗口各 20 次的专项证据；这 40 次不能合并为冻结恢复矩阵的新覆盖，见 [Stage 专项](../adr/0046-phase4-stage-effect-crash-recovery.md)。
...
| 连续纵向 | 100 Cycle，真实 Chromium/Core | [连续验收](../validation/iris-continuous.md) |
| 快照与 Stage 扩展 | 仅已列出的专项路径通过 | [恢复说明](../validation/iris-recovery.md) |

活动 Stage 的额外 HTTP/SDK ACK 组合、容量/旧快照/cursor 扩展，以及 MCP、Presence/Avatar 等后续能力集中在 [Phase 4B 待办](../plans/phase-4/backlog.md)。其余细分 Gate 未经对应证据不得标记通过。

后续执行按 [双线路线图](../plans/README.md) 分配到直播 L5–L7；游戏平台 G5–G8 独立推进，联合出口另验。本页的限定 Iris 成果不代表真实 TTS、Live2D、平台插件、Studio 或完整直播套件已经交付，产品层面见 [当前能力状态](./current-status.md)。
