...
> 实施指南：[Phase 2 开发指南](../archive/phase-2/development-guide.md)（含工作包分解与验收标准）\
> 上游基线：[Phase 1 完成态参考](./phase-1.md)\
> 后续实施：[Phase 3 开发指南](../archive/phase-3/development-guide.md)
...
- 跨进程对账以持久化活动 Scene 索引为权威（写侧同事务维护：非终态
  UPSERT、可证终态 DELETE、payload 版本校验不过保守 unknown）——
  「任一未证终态即 v2」不受记录窗口挤出影响；未知 payloadVersion
  绝不静默当作已知格式；Director admission 拒绝时预分配状态回滚（PCM
  物化推迟到 ready 之后）；maxBufferedUs=0 合法（零预算 = 只按目标
  时刻发送），迟到阈值与 Stage Deadline 等号对齐（≥ 100ms 丢弃）。
...
- ModelProvider、Signal Pipeline、Decision Loop 与 Tool Runtime 已移交
  [Phase 3 开发指南](../archive/phase-3/development-guide.md)。
