...
可信配置文件与固定启动入口已接入，见 [ADR 0041](../../../adr/0041-phase4-iris-launch-configuration.md)、[运行说明](../../../guides/iris-runtime.md) 和 [配置样例](../../../examples/iris-runtime.json)。该入口只接受凭据引用，默认支持范围与 Surface off 保持；输入身份授权、写工具装配和凭据轮换仍待完成。
