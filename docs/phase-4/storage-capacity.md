...
这些约束分别处理准入、页分配、WAL 增长和已接纳工作的收尾，不能用其中一个阈值代替其他层。配置说明见 [运行说明](../iris-runtime-operations.md)，规划中的完整容量要求见 [持久化构建分册 §9.2](./guide/persistence.md)。
...
逻辑预算没有向操作系统预分配独占磁盘块。其他进程占用空间、真实 ENOSPC、文件系统分配开销、旧备份与完整 Core/Stage 进程组合仍需独立证明。根据 [ADR 0047](../adr/0047-phase4a-recovery-scope-freeze.md)，这些扩展由 [Phase 4B 待办](../phase-4b-backlog.md) 管理，不作为冻结 Phase 4A 恢复 Gate 的前置条件。
