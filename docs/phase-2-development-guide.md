...
Phase 2 对应 [技术选型基线 §20](./technology-selection.md) 的“演出纵向链路”：用 Fake Signal 和 Fake Model 产生 ActionFrame，打通 Action Compiler、Scene Director、Stage、AudioWorklet、字幕与 Live2D Adapter，并验证同步和取消。

它只完成 [系统架构设计 §20 Milestone 1](./architecture-plan.md) 的确定性演出骨架，不宣称完成 Milestone 1 全部产品能力。真实单 Provider LLM、正式 Signal Hub、流式 TTS Provider 和平台弹幕接入属于 Phase 3 或后续纵向切片。
